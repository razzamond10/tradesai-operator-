...
export default function ClientShell({ businessName, tradeType, userName, children }: ClientShellProps) {
  ...
  const initials = userName ? userName.slice(0, 1).toUpperCase() : 'C';
        ...
        <div style={{ padding: '16px 16px 10px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>Client Portal</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '10px 14px', borderTop: '1px solid rgba(255,255,255,0.08)', position: 'relative' }} ref={profileRef}>
          {profileOpen && (
            <div style={{
              ...
              <button onClick={() => { setProfileOpen(false); router.push('/dashboard/account'); }} style={{
                display: 'flex', alignItems: 'center', gap: '9px', width: '100%',
                padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer',
                fontSize: '12px', fontWeight: 600, color: 'var(--ink)', fontFamily: '"Inter",sans-serif',
                textAlign: 'left', borderBottom: '1px solid var(--slate)',
              }}>
                <span style={{ fontSize: '14px' }}>👤</span> My Account
              </button>
