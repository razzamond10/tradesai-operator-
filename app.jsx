...
const App = () => {
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0a0512 0%, #15082a 35%, #1f0f3a 65%, #0f0520 100%)', color: '#fff', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        <header style={{ padding: '1.5rem 2rem', borderBottom: '1px solid rgba(200,150,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(180deg, rgba(25,15,45,0.8) 0%, rgba(15,5,25,0.4) 100%)', backdropFilter: 'blur(10px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <div style={{ width: '40px', height: '40px', background: 'linear-gradient(135deg, #1a5a7a 0%, #0d3f5c 100%)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', fontWeight: '700', color: '#d4af37' }}>⚙️</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0a0512 0%, #15082a 35%, #1f0f3a 65%, #0f0520 100%)', color: '#fff', fontFamily: 'system-ui, -apple-system, sans-serif', overflowX: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'fixed', width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(0,212,255,0.08) 0%, transparent 70%)', borderRadius: '50%', top: '-150px', right: '-150px', pointerEvents: 'none', filter: 'blur(40px)', zIndex: 1 }} />
      <div style={{ position: 'fixed', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(150,100,200,0.06) 0%, transparent 70%)', borderRadius: '50%', bottom: '-100px', left: '-100px', pointerEvents: 'none', filter: 'blur(40px)', zIndex: 1 }} />

      {/* Sticky Header with Logo */}
      <header style={{ padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1400px', margin: '0 auto', width: '100%', position: 'sticky', top: 0, zIndex: 50, background: isScrolled ? 'linear-gradient(180deg, rgba(15,5,25,0.95) 0%, rgba(10,5,20,0.8) 100%)' : 'transparent', backdropFilter: isScrolled ? 'blur(15px)' : 'none', transition: 'all 0.3s ease', borderBottom: isScrolled ? '1px solid rgba(150,120,200,0.1)' : 'none' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ width: '48px', height: '48px', background: 'linear-gradient(135deg, rgba(0,50,100,0.3) 0%, rgba(100,50,150,0.3) 100%)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', fontWeight: '700', color: '#d4af37', border: '1px solid rgba(150,120,200,0.2)', boxShadow: '0 8px 20px rgba(0,212,255,0.1)' }}>🔧</div>
          <div>
            <h1 style={{ fontSize: '1.15rem', fontWeight: '700', margin: 0, letterSpacing: '-0.3px' }}>Trades <span style={{ color: '#d4af37', fontWeight: '700' }}>Ai</span> Operator</h1>
            <p style={{ fontSize: '0.7rem', color: '#888', margin: '0.2rem 0 0 0' }}>24/7 AI Receptionist</p>
          </div>
        </div>
        <button onClick={() => setPage('signup')} style={{ padding: '0.65rem 1.6rem', background: 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)', color: '#000', border: 'none', borderRadius: '8px', fontSize: '0.95rem', fontWeight: '700', cursor: 'pointer', transition: 'all 0.3s ease', boxShadow: '0 8px 20px rgba(0,212,255,0.2)' }} onMouseEnter={(e) => { e.target.style.transform = 'translateY(-2px)'; e.target.style.boxShadow = '0 12px 30px rgba(0,212,255,0.3)'; }} onMouseLeave={(e) => { e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 8px 20px rgba(0,212,255,0.2)'; }}>Get Started</button>
      </header>

      {/* Hero */}
      <section style={{ padding: '4.5rem 2rem 4rem', textAlign: 'center', maxWidth: '950px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <h2 style={{ fontSize: '3.5rem', fontWeight: '800', lineHeight: '1.15', marginBottom: '1rem', letterSpacing: '-1.5px', background: 'linear-gradient(180deg, #fff 0%, #d0e8ff 100%)', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          Never Miss Another Job Again
        </h2>
        <p style={{ fontSize: '1.15rem', color: '#b8a8d8', marginBottom: '1.5rem', letterSpacing: '0.3px' }}>24/7 AI receptionist built for every UK trade business.</p>
        <p style={{ fontSize: '1.1rem', color: '#ccc', lineHeight: '1.7', marginBottom: '2.5rem', maxWidth: '700px', margin: '0 auto 2.5rem', fontWeight: '400' }}>Trades Ai Operator answers every enquiry 24/7 in clear, natural British English – always using your exact business name and tone. Calm, professional and friendly, so no customer ever reaches voicemail again.</p>
        <button onClick={() => setPage('signup')} style={{ padding: '1rem 2.5rem', background: 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)', color: '#000', border: 'none', borderRadius: '10px', fontWeight: '700', fontSize: '1.05rem', cursor: 'pointer', boxShadow: '0 20px 50px rgba(0,212,255,0.25)', transition: 'all 0.3s ease', marginRight: '1rem', marginBottom: '2rem' }} onMouseEnter={(e) => { e.target.style.transform = 'translateY(-3px)'; e.target.style.boxShadow = '0 30px 60px rgba(0,212,255,0.4)'; }} onMouseLeave={(e) => { e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 20px 50px rgba(0,212,255,0.25)'; }}>Get Started Free</button>
        <button onClick={() => setPage('signup')} style={{ padding: '1rem 2.5rem', background: 'rgba(0,212,255,0.1)', color: '#00d4ff', border: '2px solid rgba(0,212,255,0.4)', borderRadius: '10px', fontWeight: '700', fontSize: '1.05rem', cursor: 'pointer', boxShadow: '0 8px 20px rgba(0,212,255,0.1)', transition: 'all 0.3s ease', marginBottom: '2rem' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(0,212,255,0.15)'; e.target.style.boxShadow = '0 12px 30px rgba(0,212,255,0.2)'; }} onMouseLeave={(e) => { e.target.style.background = 'rgba(0,212,255,0.1)'; e.target.style.boxShadow = '0 8px 20px rgba(0,212,255,0.1)'; }}>Hear it live in 30 seconds →</button>
        
        {/* Trust Badges */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', maxWidth: '550px', margin: '0 auto', fontSize: '0.85rem', color: '#888', marginTop: '2.5rem' }}>
          <div style={{ padding: '1rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.08) 0%, rgba(100,50,150,0.08) 100%)', border: '1px solid rgba(100,150,200,0.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '0.8rem', backdropFilter: 'blur(10px)' }}>
            <span style={{ fontSize: '1.3rem' }}>🔒</span> <span style={{ fontSize: '0.8rem' }}>Secure & GDPR</span>
          </div>
          <div style={{ padding: '1rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.08) 0%, rgba(100,50,150,0.08) 100%)', border: '1px solid rgba(100,150,200,0.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '0.8rem', backdropFilter: 'blur(10px)' }}>
            <span style={{ fontSize: '1.3rem' }}>🇬🇧</span> <span style={{ fontSize: '0.8rem' }}>UK-Based</span>
          </div>
          <div style={{ padding: '1rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.08) 0%, rgba(100,50,150,0.08) 100%)', border: '1px solid rgba(100,150,200,0.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '0.8rem', backdropFilter: 'blur(10px)' }}>
            <span style={{ fontSize: '1.3rem' }}>✅</span> <span style={{ fontSize: '0.8rem' }}>Fully Insured</span>
          </div>
          <div style={{ padding: '1rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.08) 0%, rgba(100,50,150,0.08) 100%)', border: '1px solid rgba(100,150,200,0.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '0.8rem', backdropFilter: 'blur(10px)' }}>
            <span style={{ fontSize: '1.3rem' }}>💷</span> <span style={{ fontSize: '0.8rem' }}>14-Day Free</span>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        {/* Feature 1 */}
        <div style={{ padding: '2.5rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.12) 0%, rgba(100,50,150,0.12) 100%)', border: '1px solid rgba(150,120,200,0.15)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3rem', alignItems: 'center', marginBottom: '2.5rem', backdropFilter: 'blur(10px)', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
          <div style={{ fontSize: '6.5rem', textAlign: 'center', lineHeight: '1', animation: 'pulse 3s ease-in-out infinite' }}>☎️</div>
          <div>
        ...
        {/* Feature 2 */}
        <div style={{ padding: '2.5rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.12) 0%, rgba(100,50,150,0.12) 100%)', border: '1px solid rgba(150,120,200,0.15)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '3rem', alignItems: 'center', marginBottom: '2.5rem', backdropFilter: 'blur(10px)', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
          <div>
          ...
          <div style={{ fontSize: '6.5rem', textAlign: 'center', lineHeight: '1', animation: 'pulse 3s ease-in-out infinite' }}>💷</div>
        </div>
      </section>
      ...
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.8rem' }}>
          {[
            ...
            <div key={idx} style={{ padding: '2rem', background: 'linear-gradient(135deg, rgba(60,40,90,0.25) 0%, rgba(40,20,70,0.25) 100%)', border: '1px solid rgba(150,120,200,0.12)', borderRadius: '12px', backdropFilter: 'blur(10px)', transition: 'all 0.3s ease', cursor: 'pointer', transform: hoveredFeature === idx ? 'translateY(-8px)' : 'translateY(0)', boxShadow: hoveredFeature === idx ? '0 30px 60px rgba(0,212,255,0.15)' : '0 10px 30px rgba(0,0,0,0.2)' }} onMouseEnter={() => setHoveredFeature(idx)} onMouseLeave={() => setHoveredFeature(null)}>
              <div style={{ fontSize: '4rem', marginBottom: '1rem', transition: 'transform 0.3s ease', transform: hoveredFeature === idx ? 'scale(1.2)' : 'scale(1)' }}>{feature.icon}</div>
      ...
      {/* Revenue Section */}
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <div style={{ padding: '2.5rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.12) 0%, rgba(100,50,150,0.12) 100%)', border: '1px solid rgba(150,120,200,0.15)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3rem', alignItems: 'center', backdropFilter: 'blur(10px)', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
          <div style={{ fontSize: '6.5rem', textAlign: 'center', lineHeight: '1', animation: 'pulse 3s ease-in-out infinite' }}>📈</div>
          <div>
      ...
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <h2 style={{ fontSize: '2.2rem', fontWeight: '800', marginBottom: '0.8rem' }}>Simple, Transparent Pricing</h2>
          <p style={{ color: '#aaa', fontSize: '1.05rem' }}>No hidden fees. No surprise charges. Cancel anytime.</p>
        </div>
        <div style={{ padding: '2.5rem', background: 'linear-gradient(135deg, rgba(0,100,150,0.12) 0%, rgba(100,50,150,0.12) 100%)', border: '2px solid rgba(0,212,255,0.3)', borderRadius: '16px', textAlign: 'center', backdropFilter: 'blur(10px)', boxShadow: '0 20px 40px rgba(0,212,255,0.1)' }}>
          <p style={{ fontSize: '0.9rem', color: '#888', marginBottom: '1.5rem', letterSpacing: '0.5px' }}>SETUP + MONTHLY</p>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', alignItems: 'baseline', marginBottom: '2rem' }}>
            <div>
              <p style={{ fontSize: '2.2rem', fontWeight: '700', margin: 0, color: '#fff' }}>£1,197</p>
              <p style={{ fontSize: '0.85rem', color: '#888', margin: '0.5rem 0 0 0' }}>one-time setup</p>
            </div>
            <span style={{ color: '#666', fontSize: '1.5rem' }}>+</span>
            <div>
              <p style={{ fontSize: '2.2rem', fontWeight: '700', margin: 0, color: '#00d4ff' }}>£447</p>
              <p style={{ fontSize: '0.85rem', color: '#888', margin: '0.5rem 0 0 0' }}>per month</p>
            </div>
          </div>
      ...
      <section style={{ padding: '4.5rem 2rem 3.5rem', textAlign: 'center', maxWidth: '950px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <h2 style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '1rem', letterSpacing: '-0.5px' }}>Be First to Unlock Trades Ai Operator</h2>
