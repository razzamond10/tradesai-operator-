...
const App = () => {
  ...
  const features = [
    { 
      icon: '📊', 
      title: 'AUTO CRM', 
      shortDesc: 'Every interaction is logged to Google Sheets automatically, with a complete audit trail.',
      details: [
        'Every call and enquiry is logged in real-time to your Google Sheets',
        'Full audit trail for compliance and customer history review',
        'Automatic organisation by customer name, trade type, and issue',
        'Zero manual data entry — completely automated, always accurate'
      ] 
    },
    { 
      icon: '📅', 
      title: 'REAL DIARY BOOKING', 
      shortDesc: 'Checks your Google Calendar in real-time and suggests available slots automatically.',
      details: [
        'System checks your actual Google Calendar for live availability',
        'Suggests practical slots that genuinely work for your schedule',
        'Automatic SMS and WhatsApp confirmation sent to customer',
        'Your diary stays accurate — no risk of double-bookings'
      ] 
    },
    { 
      icon: '📸', 
      title: 'AI PHOTO QUOTES', 
      shortDesc: 'Customer sends a photo. Our AI analyses it and returns a realistic UK price range instantly.',
      details: [
        'Customer texts or emails a photo of the problem',
        'AI instantly analyses severity, materials, location and complexity',
        'Returns realistic UK price range based on your trade standards',
        'Leads convert faster when customers see transparent, instant quotes'
      ] 
    },
    { 
      icon: '🚨', 
      title: 'EMERGENCY PROTOCOL', 
      shortDesc: 'Gas, flooding, or electrical emergencies are detected and handled with proper safety first.',
      details: [
        'System detects emergency keywords (gas, flooding, electrical, etc.)',
        'Safety guidance is given first — before any personal details are taken',
        'Your team is alerted immediately with SMS notification and context',
        'Dangerous situations are handled with proper protocols every time'
      ] 
    }
  ];
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #050209 0%, #0f0520 20%, #1a0f35 40%, #2d1550 60%, #1f0f3a 80%, #0a0512 100%)', color: '#fff', fontFamily: 'system-ui, -apple-system, sans-serif', overflowX: 'hidden', position: 'relative' }}>
      ...
      <section style={{ padding: '4.5rem 2rem 4rem', textAlign: 'center', maxWidth: '950px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        ...
        <div style={{ display: 'flex', gap: '1.2rem', justifyContent: 'center', marginBottom: '2.5rem', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowVideo(true)} style={{ padding: '1rem 2.5rem', background: 'rgba(0,180,255,0.12)', color: '#00d4ff', border: '2px solid rgba(0,180,255,0.4)', borderRadius: '10px', fontWeight: '700', fontSize: '1.05rem', cursor: 'pointer', boxShadow: '0 8px 20px rgba(0,180,255,0.15)', transition: 'all 0.3s ease' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(0,180,255,0.18)'; e.target.style.boxShadow = '0 12px 30px rgba(0,180,255,0.25)'; }} onMouseLeave={(e) => { e.target.style.background = 'rgba(0,180,255,0.12)'; e.target.style.boxShadow = '0 8px 20px rgba(0,180,255,0.15)'; }}>Watch 42-second demo →</button>
        </div>
      ...
      <section id="demo-section" style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <div style={{ padding: '2.8rem', background: 'linear-gradient(135deg, rgba(0,120,180,0.12) 0%, rgba(120,80,180,0.12) 100%)', border: '1px solid rgba(100,180,255,0.18)', borderRadius: '16px', backdropFilter: 'blur(10px)', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
          ...
          <div style={{ padding: '1.5rem', background: 'linear-gradient(135deg, rgba(50,30,80,0.3) 0%, rgba(30,10,60,0.3) 100%)', border: '1px solid rgba(100,150,200,0.15)', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <button onClick={() => setIsPlaying(!isPlaying)} style={{ width: '60px', height: '60px', background: 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)', border: 'none', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', cursor: 'pointer', flexShrink: 0, boxShadow: '0 10px 30px rgba(0,212,255,0.3)', transition: 'all 0.3s ease' }} onMouseEnter={(e) => { e.target.style.boxShadow = '0 15px 40px rgba(0,212,255,0.45)'; }} onMouseLeave={(e) => { e.target.style.boxShadow = '0 10px 30px rgba(0,212,255,0.3)'; }}>
              {isPlaying ? '⏸' : '▶'}
            </button>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '0.9rem', color: '#ddd', marginBottom: '0.5rem' }}>"Hello, thanks for calling your plumbing business. How can I help you today?"</div>
              <div style={{ fontSize: '0.8rem', color: '#888' }}>Sample greeting (Charlotte voice via ElevenLabs)</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {showVideo && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(5px)' }} onClick={() => setShowVideo(false)}>
          <div style={{ background: '#000', borderRadius: '12px', padding: 0, maxWidth: '800px', width: '90%', boxShadow: '0 30px 60px rgba(0,0,0,0.9)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <iframe width="100%" height="450" src="https://www.youtube.com/embed/oNvSCvyfoY4?autoplay=1&controls=1" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ borderRadius: '12px' }}></iframe>
          </div>
        </div>
      )}
      ...
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        ...
        <div style={{ padding: '2.8rem', background: 'linear-gradient(135deg, rgba(0,120,180,0.12) 0%, rgba(120,80,180,0.12) 100%)', border: '1px solid rgba(100,180,255,0.18)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3.5rem', alignItems: 'center', marginBottom: '2.5rem', backdropFilter: 'blur(10px)', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
          ...
          <div>
            <h2 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '1.2rem', letterSpacing: '-0.5px' }}>Never Miss a Call</h2>
            <p style={{ color: '#bbb', lineHeight: '1.8', fontSize: '1rem', margin: '0 0 1.5rem 0' }}>Every enquiry is answered 24/7, even at 3am on a Sunday. Your customers hear a professional, friendly voice — not a voicemail. No more missed jobs.</p>
            <button onClick={() => document.getElementById('demo-section').scrollIntoView({ behavior: 'smooth' })} style={{ padding: '0.85rem 1.8rem', background: 'rgba(0,180,255,0.15)', color: '#00d4ff', border: '1px solid rgba(0,180,255,0.4)', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.95rem', transition: 'all 0.3s ease' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(0,180,255,0.25)'; }} onMouseLeave={(e) => { e.target.style.background = 'rgba(0,180,255,0.15)'; }}>Hear a live demo →</button>
          </div>
        </div>
        ...
        <div style={{ padding: '2.8rem', background: 'linear-gradient(135deg, rgba(0,120,180,0.12) 0%, rgba(120,80,180,0.12) 100%)', border: '1px solid rgba(100,180,255,0.18)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '3.5rem', alignItems: 'center', marginBottom: '2.5rem', backdropFilter: 'blur(10px)', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
          <div>
            <h2 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '1.2rem', letterSpacing: '-0.5px' }}>Instant Booking</h2>
            <p style={{ color: '#bbb', lineHeight: '1.8', fontSize: '1rem', margin: '0 0 1.5rem 0' }}>Your diary stays accurate. The system checks your real Google Calendar and books available slots. Customers get automatic SMS confirmation. No back-and-forth emails.</p>
            <button onClick={() => setShowVideo(true)} style={{ padding: '0.85rem 1.8rem', background: 'rgba(0,180,255,0.15)', color: '#00d4ff', border: '1px solid rgba(0,180,255,0.4)', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.95rem', transition: 'all 0.3s ease' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(0,180,255,0.25)'; }} onMouseLeave={(e) => { e.target.style.background = 'rgba(0,180,255,0.15)'; }}>Watch how it works →</button>
          </div>
      ...
      <section style={{ padding: '3.5rem 2rem', maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 5 }}>
        <div style={{ padding: '2.8rem', background: 'linear-gradient(135deg, rgba(0,120,180,0.12) 0%, rgba(120,80,180,0.12) 100%)', border: '1px solid rgba(100,180,255,0.18)', borderRadius: '16px', display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3.5rem', alignItems: 'center', backdropFilter: 'blur(10px)', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
          ...
          <div>
            <h2 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '1.2rem', letterSpacing: '-0.5px' }}>Make More Money</h2>
            <p style={{ color: '#bbb', lineHeight: '1.8', fontSize: '1rem', margin: '0 0 1.5rem 0' }}>Every call is logged automatically. Every quote is tracked. You see exactly what's coming in and what's booked. More calls answered = more jobs = more revenue. Simple as that.</p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              {[
                ...
                { num: '3', title: 'Your Pricing', desc: 'Get an exact quote based on your business profile' },
                { num: '4', title: 'Go Live', desc: 'Start taking calls within 48 hours of signing up' }
              ].map((step, idx) => (
