  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2F2F8', fontFamily: '"Inter",system-ui,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: '14px', padding: '40px 48px', boxShadow: '0 4px 20px rgba(26,10,60,0.10)', border: '1px solid #D8D0F0', maxWidth: '420px', width: '100%', textAlign: 'center' }}>
        <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: 'linear-gradient(135deg,#C9A84C,#E8C96A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 800, color: '#1A0A3C', fontFamily: '"Inter Tight",sans-serif', margin: '0 auto 16px' }}>
          {(user.name || 'A').slice(0, 1).toUpperCase()}
        </div>
        <div style={{ fontFamily: '"Inter Tight",sans-serif', fontSize: '20px', fontWeight: 900, color: '#1A0A3C', marginBottom: '4px' }}>My Account</div>
        <div style={{ fontSize: '12px', color: '#7468A0', marginBottom: '24px' }}>{user.name} · Owner — Admin</div>
        <div style={{ padding: '14px 16px', background: '#F2F2F8', borderRadius: '8px', fontSize: '12px', color: '#7468A0', marginBottom: '24px' }}>
          Account management coming soon. Contact support to update your credentials.
        </div>
