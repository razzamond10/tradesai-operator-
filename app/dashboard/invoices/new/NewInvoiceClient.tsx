...
interface RecentBooking { customerName: string; phone: string; jobType: string; scheduledDate: string; postcode: string; finalPrice: string }
...
export default function NewInvoiceClient({ user }: { user: User }) {
  ...
  function fillFromBooking(b: RecentBooking) {
    ...
    setBookingRef('');
    const unitPrice = parseFloat(b.finalPrice || '0') || 0;
    setLines([{ description: b.jobType || '', quantity: 1, unitPrice }]);
