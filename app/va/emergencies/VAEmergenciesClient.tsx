                  ...
                  {visible.map((em, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(0,0,0,0.04)', background: rv.label === 'Open' ? 'rgba(192,24,48,0.02)' : 'transparent' }}>
                        ...
                        <td style={{ padding: 0, fontWeight: 600 }}><Link href={`/va/emergencies/${encodeURIComponent(em.clientId + '__' + (em.timestamp || em.createdAt || ''))}`} style={{ color: 'var(--a1)', textDecoration: 'none', display: 'inline-block', width: '100%', padding: '8px 12px', boxSizing: 'border-box' }}>{em.callerName || '—'}</Link></td>
