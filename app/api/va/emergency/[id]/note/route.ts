...
export async function POST(req: NextRequest) {
  try {
    ...
    const { clientId, phone, timestamp, note } = await req.json();
    ...
    const before = rows[idx][10] || '';
    const stamp = `[${new Date().toISOString()}] ${session.email}: ${note}`;
    const after = before ? `${before}\n${stamp}` : stamp;
    const rowNum = idx + 3;
    await sheets.spreadsheets.values.update({
      spreadsheetId: config.sheetId, range: `'Emergencies'!K${rowNum}`,
      valueInputOption: 'RAW', requestBody: { values: [[after]] },
    });
    await logVAAction({
      vaEmail: session.email, vaName: session.email.split('@')[0], actionType: 'emergency.note',
    ...
    console.error('[va/emergency/note]', err);
